/**
 * Database Layer: Tasks
 *
 * This is the ONLY file that directly accesses the tasks table using ctx.db.
 * All task-related database operations are defined here as pure async functions.
 */

import { QueryCtx, MutationCtx } from "../_generated/server";
import { Id } from "../_generated/dataModel";

// CREATE
export async function createTask(
  ctx: MutationCtx,
  args: {
    userId: string;
    title: string;
    description?: string;
    status: "pending" | "in_progress" | "completed";
    priority?: "low" | "medium" | "high";
    dueDate?: number;
  }
) {
  const now = Date.now();
  return await ctx.db.insert("tasks", {
    ...args,
    createdAt: now,
    updatedAt: now,
  });
}

// READ - Get by ID
export async function getTaskById(ctx: QueryCtx, id: Id<"tasks">) {
  return await ctx.db.get(id);
}

// READ - Get by user
export async function getTasksByUser(ctx: QueryCtx, userId: string) {
  return await ctx.db
    .query("tasks")
    .withIndex("by_user", (q) => q.eq("userId", userId))
    .order("desc")
    .collect();
}

// READ - Get by user and status
export async function getTasksByUserAndStatus(
  ctx: QueryCtx,
  userId: string,
  status: "pending" | "in_progress" | "completed"
) {
  return await ctx.db
    .query("tasks")
    .withIndex("by_user_and_status", (q) =>
      q.eq("userId", userId).eq("status", status)
    )
    .order("desc")
    .collect();
}

// READ - Get by user with due date filter
export async function getTasksByUserWithDueDate(
  ctx: QueryCtx,
  userId: string,
  beforeDate?: number
) {
  const tasks = await ctx.db
    .query("tasks")
    .withIndex("by_user_and_due_date", (q) => q.eq("userId", userId))
    .collect();

  // Filter by due date if provided
  if (beforeDate !== undefined) {
    return tasks.filter(
      (task) => task.dueDate !== undefined && task.dueDate <= beforeDate
    );
  }

  return tasks;
}

// READ - Get upcoming tasks (not completed, with due date)
export async function getUpcomingTasks(ctx: QueryCtx, userId: string) {
  const allTasks = await ctx.db
    .query("tasks")
    .withIndex("by_user", (q) => q.eq("userId", userId))
    .collect();

  const now = Date.now();

  return allTasks
    .filter(
      (task) =>
        task.status !== "completed" &&
        task.dueDate !== undefined &&
        task.dueDate > now
    )
    .sort((a, b) => (a.dueDate || 0) - (b.dueDate || 0));
}

// READ - Get overdue tasks
export async function getOverdueTasks(ctx: QueryCtx, userId: string) {
  const allTasks = await ctx.db
    .query("tasks")
    .withIndex("by_user", (q) => q.eq("userId", userId))
    .collect();

  const now = Date.now();

  return allTasks.filter(
    (task) =>
      task.status !== "completed" &&
      task.dueDate !== undefined &&
      task.dueDate < now
  );
}

// READ - Count tasks by status for user
export async function countTasksByStatus(ctx: QueryCtx, userId: string) {
  const tasks = await getTasksByUser(ctx, userId);

  return {
    pending: tasks.filter((t) => t.status === "pending").length,
    in_progress: tasks.filter((t) => t.status === "in_progress").length,
    completed: tasks.filter((t) => t.status === "completed").length,
    total: tasks.length,
  };
}

// UPDATE
export async function updateTask(
  ctx: MutationCtx,
  id: Id<"tasks">,
  args: {
    title?: string;
    description?: string;
    status?: "pending" | "in_progress" | "completed";
    priority?: "low" | "medium" | "high";
    dueDate?: number;
    completedAt?: number;
  }
) {
  return await ctx.db.patch(id, {
    ...args,
    updatedAt: Date.now(),
  });
}

// UPDATE - Mark as completed
export async function markTaskCompleted(ctx: MutationCtx, id: Id<"tasks">) {
  const now = Date.now();
  return await ctx.db.patch(id, {
    status: "completed",
    completedAt: now,
    updatedAt: now,
  });
}

// UPDATE - Mark as in progress
export async function markTaskInProgress(ctx: MutationCtx, id: Id<"tasks">) {
  return await ctx.db.patch(id, {
    status: "in_progress",
    updatedAt: Date.now(),
  });
}

// DELETE
export async function deleteTask(ctx: MutationCtx, id: Id<"tasks">) {
  return await ctx.db.delete(id);
}